...
pplx::task<utility::string_t> NetworkManager::get(const std::string& url)
{
    ...
    return client.request(request).then([](web::http::http_response response) -> pplx::task<utility::string_t> {
...
pplx::task<utility::string_t> NetworkManager::rollback(const std::string& url)
{
    web::http::client::http_client client(U(url));
    web::http::http_request request(web::http::methods::GET);
    return client.request(request).then([](web::http::http_response response) -> pplx::task<utility::string_t> {
        ...
        {
            std::cerr << "Error en la solicitud ROLLBACK." << std::endl;
            return pplx::task_from_result(utility::string_t(U("")));
        }
    });
}
